...
You can also store these files in a custom directory by setting the `MEETINGS_CONFIG_DIR` environment variable, though the tool currently reads templates from the path configured in `src/config.mjs` (the `templates/` directory bundled with the package).
