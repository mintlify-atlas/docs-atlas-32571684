...
<Note>
  `HACKMD_TEAM_NAME` is configured per meeting group inside the `meeting_base_<shortname>` template file, not as a global environment variable. See [Meeting base configuration](/configuration/meeting-base) for details.
</Note>
